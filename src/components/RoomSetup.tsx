...
export default function RoomSetup({ state, onAddRoom, onUpdateRoom }: Props) {
  ...
  function validate() {
    ...
    if (form.durationMinutes < 1 || form.durationMinutes > 240) e.durationMinutes = 'Duration must be 1–240 minutes.';
    return e;
  }
  ...
  function handleCancel() { setEditing(null); setAdding(false); setErrors({}); }

  function handleSaveNew() {
    const e = validate();
    if (Object.keys(e).length > 0) { setErrors(e); return; }
    const now = new Date().toISOString();
    onAddRoom({ id: `room_${Date.now()}`, ...form, createdAt: now, updatedAt: now });
    setAdding(false); setForm(emptyRoom());
  }
  ...
  function handleSaveEdit() {
    ...
    if (!existing) return;
    onUpdateRoom({ ...existing, ...form, updatedAt: new Date().toISOString() });
    setEditing(null);
  }
      ...
      <div className="bg-slate-800/80 border border-slate-600 rounded-xl p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" min={1} max={240} className={`w-full bg-slate-700 border ${errors.durationMinutes ? 'border-red-500' : 'border-slate-600'} rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-blue-500`} value={form.durationMinutes} onChange={(e) => setForm((f) => ({ ...f, durationMinutes: parseInt(e.target.value) || 60 }))} />
          ...
          <div>
            ...
            <select className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-blue-500" value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as RoomStatus }))}>
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onSave} className="flex items-center gap-2 px-4 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors"><Save className="w-4 h-4" /> Save Room</button>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Create and manage your escape rooms. Each room holds its own scripts, hint ladders, and pronunciation guide.</p>
        </div>
      ...
      <div className="space-y-4">
        {state.rooms.map((room) => {
          ...
          const hintCount = state.hintLadders.filter((h) => h.roomId === room.id).length;
          return (
            <div key={room.id}>
              {isEditing ? renderForm(handleSaveEdit) : (
                <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-5">
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => handleEdit(room)} className="ml-4 p-2 text-slate-400 hover:text-slate-200 hover:bg-slate-700 rounded-lg transition-colors"><Edit2 className="w-4 h-4" /></button>
                  </div>
                  <div className="flex flex-wrap items-center gap-4 mt-4 text-xs text-slate-400">
                    ...
                    <span className="flex items-center gap-1.5">{hintCount} hint ladder{hintCount !== 1 ? 's' : ''}</span>
                  </div>
                  {room.notes && (
