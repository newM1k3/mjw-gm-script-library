...
export const GMS_SOURCE_APP = 'Scripts';
...
function roomIntegrationHints(): Record<string, unknown> {
  return {
    ...
    puzzleFlowUse: 'Associate hint ladders with puzzle flow stages. Import stage labels from Flow to auto-scaffold hint ladder entries.',
    ...
    mjwOperatorToolkitUse: 'Bundle with Ready for a unified pre-shift readiness and script consistency dashboard.',
...
export function exportRoomMarkdown(state: AppState, roomId: string): string {
  ...
  lines.push(`# Scripts — Room Script Packet`);
  ...
  lines.push('---');
  lines.push('');
  lines.push('*Generated by Scripts · MJW Personal App Platform*');
  lines.push('');
  lines.push('**Future Integration Note:** This packet can be imported into Ready for pre-shift acknowledgement tasks, and into Flow for stage-based hint ladder alignment.');
...
export function exportAcknowledgementReportMarkdown(state: AppState, filters: AcknowledgementReportFilters = {}): string {
  ...
  lines.push('# Scripts — Staff Acknowledgement Compliance Report');
  lines.push(`**Schema Version:** ${GMS_EXPORT_SCHEMA_VERSION}`);
...
export function exportReadinessMarkdown(
  ...
  lines.push('# Scripts — Readiness Audit Report');
  lines.push(`**Schema Version:** ${GMS_EXPORT_SCHEMA_VERSION}`);
  lines.push(`**Exported:** ${new Date().toLocaleString()}`);
...
export function buildFullBackupPayload(state: AppState) {
  return {
    ...
    restoreGuidance: {
      safeDefault: 'Import room packets first for scoped restore. Use full backup restore only from a trusted Scripts export.',
...
export function previewRoomPacketImport(rawJson: string, currentState: AppState): RoomPacketImportPreview {
  ...
  if (!packet) {
    errors.push('This JSON is not a Scripts room_packet export.');
  }
